...
const handleViewStaff = (staffId: string) => {
  ...
  return (
    <div className="h-[calc(100vh-100px)] bg-[var(--color-background)] text-[var(--color-foreground)] p-4 md:p-6 w-1/1 overflow-auto custom-scrollbar">
      <div className="w-full  mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-[var(--color-foreground)]">Staff Management</h1>

        {/* Summary Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 w-1/1">
          <SummaryCard
            title="Total Staff"
            value={totalStaff}
            icon={<svg className="w-4/12 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17 20h-2v-2a4 4 0 00-4-4H9a4 4 0 00-4 4v2H3m14 0a2 2 0 102 2H3a2 2 0 102-2h14zm-7 0h-2m0-7a4 4 0 11-8 0 4 4 0 018 0z"></path></svg>}
            bgColor="bg-blue-100"
          ...
          <SummaryCard
            title="Active Staff"
            value={activeStaff}
            icon={<svg className="w-4 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>}
        ...
        {/* Staff List Table */}
        <div className="bg-[var(--color-card-background)] rounded-lg shadow-md overflow-hidden border border-[var(--color-border)] w-1/1">
          ...
          <div className="overflow-x-auto custom-scrollbar">
            <table className="min-w-full divide-y divide-[var(--color-border)]">
              <thead className="bg-[var(--color-muted-background)]">
                <tr>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Image</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Name</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Role</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Email</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Phone</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Status</th>
                  <th className="px-2 py-3 text-left text-xs font-medium text-[var(--color-muted-foreground)] uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
              <tbody className="bg-[var(--color-card-background)] divide-y divide-[var(--color-border)]">
                {staffList.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="px-2 py-4 text-center text-[var(--color-muted-foreground)]">
                  ...
                  staffList.map((staff) => (
                    <tr key={staff.id} className="hover:bg-[var(--color-muted-background)] transition-colors duration-150">
                      ...
                      <td className="px-2 py-4 whitespace-nowrap text-[var(--color-foreground)]">{staff.name}</td>
                      <td className="px-2 py-4 whitespace-nowrap text-[var(--color-foreground)]">{staff.role}</td>
                      <td className="px-2 py-4 whitespace-nowrap text-[var(--color-foreground)]">{staff.email}</td>
                      <td className="px-2 py-4 whitespace-nowrap text-[var(--color-foreground)]">{staff.phone}</td>
                      <td className="px-2 py-4 whitespace-nowrap">
